...
import (
	...
	integrationutils "github.com/atoshi-chain/atoshi/v20/testutil/integration/evmos/utils"
...
var _ = Describe("when sending a Cosmos transaction", Label("AnteHandler"), Ordered, func() {
	...
	Context("and the sender account has enough balance to pay for the transaction cost", Ordered, func() {
		...
		BeforeEach(func() {
			...
			msg = &banktypes.MsgSend{
				...
				ToAddress:   "atoshi1dx67l23hz9l0k9hcher8xz04uj7wf3yu26l2yn",
	...
	Context("and the sender account neither has enough balance nor sufficient staking rewards to pay for the transaction cost", func() {
		BeforeEach(func() {
			...
			// Transfer 1 aatos to this account so it is
			// added on chain
	...
	Context("and the sender account has not enough balance but sufficient staking rewards to pay for the transaction cost", func() {
		// minExpRewards are the minimun rewards that should be accrued
		// for the test case
		minExpRewards := sdk.DecCoins{sdk.DecCoin{Amount: math.LegacyNewDec(1e8), Denom: s.network.GetBaseDenom()}}
		...
		BeforeEach(func() {
			...
			err = s.factory.Delegate(priv, valAddr, sdk.NewCoin(s.network.GetBaseDenom(), math.NewInt(9888e14)))
			Expect(err).To(BeNil())
			...
			msg = &banktypes.MsgSend{
				FromAddress: addr.String(),
				ToAddress:   "atoshi1dx67l23hz9l0k9hcher8xz04uj7wf3yu26l2yn",
				Amount:      sdk.Coins{sdk.Coin{Amount: math.NewInt(1), Denom: s.network.GetBaseDenom()}},
			}
		})
	})
})
